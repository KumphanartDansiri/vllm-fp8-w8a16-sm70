#!/usr/bin/env bash
# Measure 122B-A10B-FP8 — run AFTER tools/hl_serve_122b.sh shows "Application
# startup complete". 1 warmup + 5 timed curls; result -> file + stdout.
set -uo pipefail
PORT="${PORT:-8002}"
MODEL=/mnt/models/Qwen3.5-122B-A10B-FP8
LABEL="122B-A10B-FP8 ns=8 TP=8"
RES=/tmp/v100_bench/RESULT_hl_122b.txt
...
if ! curl -sf "http://localhost:${PORT}/health" >/dev/null 2>&1; then
    echo "[hl] no healthy serve on :$PORT — start tools/hl_serve_122b.sh first"; exit 1
fi
