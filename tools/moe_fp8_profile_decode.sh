...
echo "[prof] healthy; decoding $GENTOK tokens single-stream (M=1) to fill profiler"
curl -s "http://localhost:${PORT}/v1/chat/completions" -H 'Content-Type: application/json' \
  -d "{\"model\":\"$SERVED\",\"stream\":false,\"max_tokens\":$GENTOK,\"temperature\":0,\"ignore_eos\":true,\"messages\":[{\"role\":\"user\",\"content\":\"Write a long detailed essay about France.\"}]}" >/dev/null 2>&1 || true
sleep 3
