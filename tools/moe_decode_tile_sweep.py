...
def main():
    ...
    MS = [int(m) for m in os.environ.get("SWEEP_M", "1").split(",")]
    log(f"shapes: E={E} topk={TOPK} K={K} Nshard={NSHARD} Ms={MS}")
    N13 = 2 * NSHARD
    torch.manual_seed(0)
    ...
    def run_variant(label, cfg, M, x, w, ids):
        if cfg is None:
        ...
        try:
            ...
            log(f"M={M:<3d} {label:36s}: {ms * 1e3:8.1f} us")
            return ms
        except Exception as ex:
            log(f"M={M:<3d} {label:36s}: FAIL {type(ex).__name__}: {str(ex)[:90]}")
            return None

    variants = [
        ("16/64/128 w4 s2", dict(BLOCK_SIZE_M=16, BLOCK_SIZE_N=64, BLOCK_SIZE_K=128, GROUP_SIZE_M=1, num_warps=4, num_stages=2)),
        ("16/32/64  w4 s2 (kbest)", dict(BLOCK_SIZE_M=16, BLOCK_SIZE_N=32, BLOCK_SIZE_K=64, GROUP_SIZE_M=1, num_warps=4, num_stages=2)),
    ...
    for M in MS:
        ...
        base = run_variant("default (16/64/128 w4 s4)", None, M, x, w, ids)
        best = (base, "default") if base else (None, None)
        for label, cfg in variants:
            ms = run_variant(label, cfg, M, x, w, ids)
            if ms is not None and (best[0] is None or ms < best[0]):
                best = (ms, label)

        # floor = active expert weight bytes (unique experts touched, capped at E)
        uniq = min(M * TOPK, E)
        floor_us = uniq * (N13 * K + K * NSHARD) * 2 / 816e9 * 1e6
        if base and best[0]:
            log(f"M={M:<3d} best = {best[1]} at {best[0] * 1e3:.1f} us "
                f"({base / best[0]:.2f}x vs default; floor ~{floor_us:.0f} us, "
                f"{best[0] * 1e3 / floor_us:.0f}x off)\n")
    log("DONE")
